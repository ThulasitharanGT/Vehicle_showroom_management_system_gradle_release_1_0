...
//creating Error DataFrame
    ...
    if (scn_no ==1)
    {
      Scn_df = DFMap("%df1%").filter(ArgMap("%where_condition%").toString).select(ArgMap("*").toString)
      Scn_df
    }
    ...
    //val DF_final= DFMap("df1").as("a").drop(ArgMap("%drop_coloumn_a%").toString).join(DFMap("df2").as("b").drop(ArgMap("%drop_coloumn_b%").toString),ArgMap("%join_condition%").toString).select(ArgMap("*").toString).filter(ArgMap("%where_condition%").toString)
      Scn_df= (DFMap("%df1%").drop(ArgMap("%drop_a_condition%"))).as("a").join((DFMap("%df2%").drop(ArgMap("%drop_b_condition%"))).as("b"),List_map("%join_condition%"),ArgMap("%join_type%").toString).select(ArgMap("*").toString).filter(ArgMap("%where_condition%").toString)
      Scn_df
    }
    ...
    {
      Scn_df= (DFMap("%df1%").drop(ArgMap("%drop_a_condition%")).as("a")).join((DFMap("%df2%").drop(ArgMap("%drop_b_condition%"))).as("b"),List_map("%join_condition%")).select(ArgMap("*").toString).filter(ArgMap("%where_condition%").toString)
      Scn_df
    }
    else
    {
